...
def get_temperature(city, state):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if temperature_str.endswith("°C") or temperature_str.endswith("°F"):
                temperature = temperature_str[:-2].strip()  # Remove the unit (°C or °F)
                if temperature_str.endswith("°C"):
                    # Convert Celsius to Fahrenheit
                    temperature = str(round(float(temperature) * 9/5 + 32))
                return temperature
            else:
...
if __name__ == "__main__":
  ...
  if city and state:
    ...
    if temperature is not None:
      print(f"The current temperature in {city}, {state} is {temperature}°F.")
    else:
